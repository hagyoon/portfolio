...
export default function Nav(){
  ...
  return <><header className="site-nav"><div className="container-edge nav-inner">
    ...
    <div className="nav-actions"><div className="desktop-preferences"><ThemeToggle/></div><Link className="nav-studio" href="/admin" aria-label="Open Studio to edit the site"><span aria-hidden="true">◆</span> Studio</Link><a className="nav-contact" href="/#contact">Let’s talk <span aria-hidden="true">↗</span></a><button className="menu-trigger" ref={trigger} aria-label="Open menu" aria-expanded={open} aria-controls="mobile-menu" onClick={()=>setOpen(true)}><span/><span/></button></div>
  </div></header>{open&&<div className="mobile-menu" ref={menu} id="mobile-menu" role="dialog" aria-modal="true" aria-label="Navigation"><div className="mobile-menu-top"><span className="brand">hkryu.</span><button onClick={()=>{setOpen(false);trigger.current?.focus();}} aria-label="Close menu">×</button></div><nav aria-label="Mobile primary">{links.map((l,i)=><Link key={l.href} href={l.href} onClick={()=>setOpen(false)}><span>0{i+1}</span>{l.label}<span>↗</span></Link>)}<Link href="/admin" onClick={()=>setOpen(false)}><span>06</span>Studio<span>◆</span></Link></nav><div className="mobile-preferences"><ThemeToggle/><MotionToggle/></div><p className="eyebrow">Independent practice · Singapore</p></div>}</>;
}
